...
def verify_function(upper_and_lower_cone_id):
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            dbquery=""" 
                        SELECT upper_and_lower_cone FROM fork_and_suspension_assemblies 
                        WHERE fork_and_suspension_assemblies.upper_and_lower_cone IN 
                        (SELECT id FROM upper_and_lower_cones WHERE upper_and_lower_cone_id= %s )
                    """
