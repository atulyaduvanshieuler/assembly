    ...
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            if verification[0]:
                ...
                dbquery_1 = """
                                INSERT INTO biws(biw_id)
                                VALUES(%s)
                            """
                cur.execute(dbquery_1, (biw_id,))
                conn.commit()

                dbquery_2="""
                                UPDATE vehicles SET biw = sub_query.id FROM
                                (SELECT id FROM biws WHERE biws.biw_id=%s) AS sub_query  
                                WHERE vehicles.emch= %s 
                            """
                cur.execute(dbquery_2,(biw_id,emch_number,))
                conn.commit()

                msg = {
                    "message": "biw with biw id: %s added successfully in vehicle with following EMCH number: %s"
                    % (biw_id, emch_number)
                }
            else:
