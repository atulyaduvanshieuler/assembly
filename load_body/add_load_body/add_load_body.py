    ...
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            if verification[0]:
                ...
                dbquery_1 = """
                                INSERT INTO load_bodies(load_body_id)
                                VALUES(%s)
                            """
                cur.execute(dbquery_1, (load_body_id,))
                conn.commit()

                dbquery_2="""
                                UPDATE vehicles SET load_body = sub_query.id FROM
                                (SELECT id FROM load_bodies WHERE load_bodies.load_body_id=%s) AS sub_query  
                                WHERE vehicles.emch= %s 
                            """
                cur.execute(dbquery_2,(load_body_id,emch_number,))
                conn.commit()

                msg = {
                    "message": "load_body with load_body id: %s added successfully in vehicle with following EMCH number: %s"
