...
def verify_function(motor_controller_id):
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            dbquery=""" 
                        SELECT {component_key_in_subassembly_table} FROM cocktail_box_assemblies WHERE cocktail_box_assemblies.{component_key_in_subassembly_table} IN (SELECT id FROM {component_table_name} WHERE motor_controller_id= %s )
                    """
            ...
            dbquery_2="""
                          SELECT motor_controller_id FROM {component_table_name} WHERE motor_controller_id=%s
                      """
