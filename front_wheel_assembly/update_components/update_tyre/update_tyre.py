    ...
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            if verification[0]:
                ...
                dbquery="""
                            UPDATE front_wheel_assemblies 
                            SET tyre=sub_query_1.id FROM
                            (SELECT id FROM tyres WHERE tyre_id=%s) AS sub_query_1,
                            (SELECT front_wheel_assembly FROM vehicles WHERE emch=%s) AS sub_query_2
                            WHERE front_wheel_assemblies.id=sub_query_2.front_wheel_assembly
                        """
