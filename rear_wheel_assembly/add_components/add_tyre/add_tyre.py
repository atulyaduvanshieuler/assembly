    ...
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            if verification[0]:
                ...
                dbquery_1 = """INSERT INTO rear_wheel_assemblies(tyre) SELECT id FROM tyres WHERE tyre_id = (%s)"""
                cur.execute(dbquery_1, (tyre_id,))
                conn.commit()

                dbquery_2="""UPDATE vehicles SET rear_wheel_assembly = sub_query_1.id FROM
                (SELECT id FROM rear_wheel_assemblies WHERE rear_wheel_assemblies.tyre IN
