...
def verify_function(signal_connector_id):
    with psycopg2.connect(**database_config) as conn: 
        with conn.cursor() as cur:
            ...
            dbquery=""" 
                        SELECT signal_connector FROM battery_assemblies WHERE battery_assemblies.signal_connector IN (SELECT id FROM signal_connectors WHERE signal_connector_id= %s )
                    """
            ...
            dbquery_2="""
                          SELECT signal_connector_id FROM signal_connectors WHERE signal_connector_id=%s
                      """
