        ...
        with conn.cursor() as cur:
            ...
            if verification[0]:
                ...
                dbquery="""
                            ...
                            SET bms=sub_query_1.id FROM
                            (SELECT id FROM bmses WHERE bms_id=%s) AS sub_query_1,
                ...
                cur.execute(dbquery,(bms_id,emch_number,))
                conn.commit()

                msg = {
                    "message": "bms with bms id: %s updated successfully in vehicle with following EMCH number: %s"
                    % (bms_id, emch_number)
                }
            else:
